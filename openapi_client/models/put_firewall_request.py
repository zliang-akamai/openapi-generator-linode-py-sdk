...
class PutFirewallRequest(BaseModel):
    ...
    status: Optional[StrictStr] = Field(default=None, description="The status to be applied to this Firewall.   - When a Firewall is first created its status is `enabled`.  - Run the [Delete a firewall](https://techdocs.akamai.com/linode-api/reference/delete-firewall) operation to delete a Firewall.")
    ...
    __properties: ClassVar[List[str]] = ["label", "status", "tags"]
