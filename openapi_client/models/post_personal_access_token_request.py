...
class PostPersonalAccessTokenRequest(BaseModel):
    """
    PostPersonalAccessTokenRequest
    """ # noqa: E501
    expiry: Optional[datetime] = Field(default=None, description="When this token should be valid until.  If omitted, the new token will be valid until it is manually revoked.")
    ...
    scopes: Optional[StrictStr] = Field(default=None, description="The access [scopes](https://techdocs.akamai.com/linode-api/reference/get-started#oauth-reference) to grant to the created token. These cannot be changed after creation, and may not exceed the scopes of the acting token.  If omitted or entered with a wildcard character (`*`), the new token will have the same scopes as the acting token.  Multiple scopes are separated by a space character (` `).  For example, `linodes:read_write account:read_only`.")
    __properties: ClassVar[List[str]] = ["expiry", "label", "scopes"]
